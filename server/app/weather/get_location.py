...
def _polygon_centroid(ring: list[list[float]]) -> tuple[float, float]:
    ...
    # coords are [lon, lat] -> cx is centroid longitude,
    # cy is centroid latitude
    return cy, cx
...
def get_first_field_coords(db: Session, user_id: int) -> tuple[float, float]:
    ...
    if geom_type == "Polygon":
        ...
        raise ValueError(
            f"Unsupported boundary geometry type: '{geom_type}'."
        )
...
def get_field_coords(
    ...
    """
    Fetch the boundary polygon from a SPECIFIC field (not just the
    user's first one) and return its centroid as (lat, lon).

    Used for per-field health scoring, where each field needs its
    own local weather, not just whichever field was created first.
    """

    field = (
        db.query(FieldForm)
        .filter(
            FieldForm.id == field_id,
    ...
    if not field:
        raise ValueError("Field not found for this user.")
    ...
    if not boundary or "coordinates" not in boundary or "type" not in boundary:
        raise ValueError(
            f"Field '{field.field_name}' has no boundary data."
        )
