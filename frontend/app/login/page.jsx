...
export default function LoginPage(){
  ...
  return <AuthShell mode="login">{step===1 ? <div>
    ...
    <button disabled={mobile.length!==10||loading} onClick={submitMobile} className="btn-primary mt-6 w-full disabled:cursor-not-allowed disabled:opacity-40"><ArrowRight size={18}/>{loading?"...":t.continue}</button>
  ...
  </div> : <OtpForm mode="login" mobile={mobile} initialOtp={initialOtp} onBack={()=>setStep(1)}/>}</AuthShell>
}