...
export default function DashboardOverview() {
  ...
  useEffect(() => {
    async function fetchDashboardData() {
      try {
        ...
        // Extract array from {"status": "success", "data": [...]}
  ...
  return (
    <div className="space-y-5 pb-6">
      ...
      <section className="grid grid-cols-2 gap-3 xl:grid-cols-4">
        {loading ? (
          <div className="col-span-full rounded-2xl border border-slate-200 bg-white p-5 text-center font-bold text-slate-500">
            Loading metrics...
          </div>
        ) : error ? (
          <div className="col-span-full rounded-2xl border border-red-200 bg-red-50 p-5 text-center font-bold text-red-600">
            Error: {error}
          </div>
        ) : !metrics ? (
          <div className="col-span-full rounded-2xl border border-slate-200 bg-white p-5 text-center font-bold text-slate-500">
            No metrics available in database.
          </div>
        ) : (
          <>
            <MetricCard
              label={t.overallCropHealth || "Crop Health"}
              value={metrics.crop_health ?? "N/A"}
              suffix="/100"
              sub={t.good || "Good"}
              icon={<HealthRing score={metrics.crop_health || 0} />}
            />
            <MetricCard
              label={t.totalFields || "Total Fields"}
              value={metrics.total_fields ?? 0}
              sub={t.activeFields || "Active"}
              icon={
            ...
            <MetricCard
              ...
              sub={t.requiresAttention || "Attention Needed"}
              icon={
            ...
            />
            <MetricCard
              label={t.nextIrrigation || "Next Irrigation"}
              value={metrics.next_irrigation ?? 0}
              suffix={` ${t.days || "Days"}`}
              sub={t.northField || "Scheduled"}
              icon={
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1.45fr)_minmax(300px,.75fr)]">
        <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <Link href="/satellite" className="inline-flex min-h-11 items-center gap-1 rounded-xl px-3 text-sm font-bold text-emerald-700 hover:bg-emerald-50">
            ...
            <div className="flex rounded-xl bg-slate-100 p-1">
              {layers.map(key => (
                <button
                  ...
                  className={`min-h-11 rounded-lg px-3 text-xs font-bold sm:px-4 ${layer === key ? "bg-white text-emerald-700 shadow-sm" : "text-slate-600"}`}
                >
                  {t[`layer${key === "ndvi" ? "Ndvi" : key === "trueColor" ? "TrueColor" : "Moisture"}`]}
                </button>
              ))}
            </div>
            <div className="flex gap-2">
              <button type="button" onClick={zoomIn} disabled={zoom>=2.5} className="flex h-11 w-11 items-center justify-center rounded-xl border border-slate-200 transition hover:border-emerald-300 hover:text-emerald-700 disabled:cursor-not-allowed disabled:opacity-40">
                <ZoomIn size={18}/>
              </button>
              <button type="button" onClick={zoomOut} disabled={zoom<=1} className="flex h-11 w-11 items-center justify-center rounded-xl border border-slate-200 transition hover:border-emerald-300 hover:text-emerald-700 disabled:cursor-not-allowed disabled:opacity-40">
                <ZoomOut size={18}/>
              </button>
            </div>
          </div>
...
function HealthRing({ score }) {
  return (
    <div className="flex h-11 w-11 items-center justify-center rounded-full" style={{ background: `conic-gradient(#22c55e ${score * 3.6}deg, #dcfce7 0deg)` }}>
      <div className="flex h-8 w-8 items-center justify-center rounded-full bg-white text-[10px] font-black text-emerald-700">
        {score}
      </div>
    </div>
  );
