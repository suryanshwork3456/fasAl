...
export default function FieldDetail({ id }) {
  ...
  // Restored — this is what feeds the real 3x5 grid into FieldMap.
  // Allowed to fail quietly: a missing/errored NDVI grid shouldn't
  // blank out the rest of the page, the map just falls back to demo.
  useEffect(() => {
    async function loadNdvi() {
      try {
        const data = await authFetch(`/api/v1/fields/${id}/ndvi`);
        setNdvi(data);
      } catch (err) {
        console.error("NDVI grid fetch failed:", err);
        setNdvi(null);
      }
    }
    loadNdvi();
  ...
  useEffect(() => {
    async function loadHealthScore() {
      try {
        const data = await authFetch(`/api/v1/fields/${id}/health-score`);
        setHealth(data);
      } catch (err) {
